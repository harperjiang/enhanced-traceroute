...
public class Ping {
	...
	public static interface EchoReplyListener {
		public void notifyEchoReply(ICMPEchoPacket packet, byte[] data,
				int dataOffset, byte[] srcAddress) throws IOException;
	}
	...
	public static class Pinger {
		...
		public void sendEchoRequest(InetAddress host) throws IOException {
			...
			OctetConverter
					.longToOctets(System.nanoTime(), sendData, dataOffset);
		...
		/**
		 * @return The number of bytes in the data portion of the ICMP ping
		 *         request packet.
		 */
	...
	public static class PingerIPv6 extends Pinger {
		 ...
		 * checksum for the sender, but Microsoft's IPv6 stack does not do this.
		 * Nor does it support the IPV6_CHECKSUM socket option. Therefore, in
		 * order to work on the Windows family of operating systems, we have to
		...
		private static class ICMPv6ChecksumCalculator extends IPPacket {
			...
			private int computeVirtualHeaderTotal(byte[] destination,
					byte[] source, int icmpLength) {
				int total = 0;
			...
			int computeChecksum(byte[] data, ICMPPacket packet,
		...
		public PingerIPv6(int id) throws IOException {
			super(id, PF_INET6, IPPROTO_ICMPV6 /*
												 * getProtocolByName("ipv6-icmp")
												 */);
			...
			ping.setEchoReplyListener(new EchoReplyListener() {
				...
				public void notifyEchoReply(ICMPEchoPacket packet, byte[] data,
					...
					buffer.append(packet.getICMPPacketByteLength())
							.append(" bytes from ").append(hostname)
							.append(" (");
					buffer.append(InetAddress.getByAddress(srcAddress)
							.toString());
					buffer.append("): icmp_seq=")
							.append(packet.getSequenceNumber()).append(" ttl=")
							.append(packet.getTTL()).append(" time=")
